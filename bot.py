...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("📝 О чем тебе напомнить? Напиши текст напоминания:")
...
async def process_hour(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏱ Выбери минуты:", reply_markup=get_time_keyboard(m_list, "min"))
