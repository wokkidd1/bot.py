...
async def process_minute(callback: types.CallbackQuery, state: FSMContext):
    ...
    dt_str = f"{user_data['selected_date']} {user_data['selected_hour']}:{minute_val}"
    
    # Создаем объект времени и привязываем его к Москве (делаем "умным")
    naive_dt = datetime.strptime(dt_str, "%Y-%m-%d %H:%M")
    remind_at = MOSCOW_TZ.localize(naive_dt)

    # Проверяем относительно текущего времени в Москве
    ...
    await callback.message.edit_text(
        f"✅ Напоминание создано!\n⏰ {remind_at.strftime('%d.%m.%Y %H:%M')} (МСК)\n📍 {user_data['reminder_text']}"
    )
    await state.clear()
